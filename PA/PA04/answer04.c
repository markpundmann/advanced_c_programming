 ...
 */
{
  ...
  while(fgets(buffer, LEN_LINE+1, fin) != NULL)
  {
    char *temp=buffer;
    while((temp=strstr(temp, str)) != NULL)
    {
      counter++;
      temp++;
  ...
  fprintf(fout, "%s appears %d times. \n", str, counter);
 ...
 */
{
  ...
  long front_index = 0;
  long back_index = fsize-1;

  while(front_index < back_index)
  {
    long front_letter = read_front_letter(fin, &front_index);
    long back_letter = read_back_letter(fin, &back_index);
    
    if(front_letter!=back_letter)
    {
      fprintf(fout, "not palindrome\n");
      fclose(fin);
      fclose(fout);
      return EXIT_SUCCESS;
    }
